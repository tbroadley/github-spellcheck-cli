...
exports.generateWordDiff = (before, after) => {
  ...
  assert(removedIndex === addedIndex - 1);

  const removed = diff[removedIndex];
  const added = diff[addedIndex];

  const removedWords = _.split(_.trimEnd(removed.value), ' ');
  const addedWords = _.split(_.trimEnd(added.value), ' ');

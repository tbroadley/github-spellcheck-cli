...
function getColumnWidth(header, commands, columnFunction) {
  return _(commands).map(columnFunction).concat([header]).map(_.size).max();
}
...
function printHelp(commands) {
  ...
  const columnWidths = _.map(COLUMN_HEADERS, (header, index) => {
    return getColumnWidth(header, commands, COLUMN_FUNCTIONS[index]);
  });

  const allCommands = _.concat(commands, HELP_COMMAND);

  const { command: exampleCommand } = _.first(commands);
  console.log(`Any prefix of a command will match that command. For example, "${exampleCommand.slice(0, 1)}" will match "${exampleCommand}".`);
  console.log();

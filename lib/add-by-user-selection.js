...
exports.addByUserSelection = async (misspellings, repo) => {
  ...
  for (let index = 0; index < misspellings.length; index += 1) {
    ...
    await respondToUserInput(
      'Apply this correction? y(es), n(o), r(emove duplicate word), i(gnore word), w(hitelist word), e(dit), s(kip file), h(elp): ',
      [
        {
          regex: /^y(es)?$/,
        ...
        },
        {
          regex: /^h(elp)?$/,
          responseFunction: () => {
            console.log('y(es)                     Include this correction in the pull request.');
            console.log('n(o)                      Do not include this correction in the pull request.');
            console.log('r(emove duplicate word)   Remove the word to be corrected and the space character in front of it.');
            console.log('i(gnore word)             Ignore the word to be corrected for the rest of the session.');
            console.log('w(hitelist word)          Permanently whitelist the word to be corrected.');
            console.log('e(dit)                    Replace the word to be corrected with a specified word.');
            console.log('s(kip file)               Do not include this corrections or any other corrections in the file.');
            console.log('h(elp)                    Print this usage guide.');
          },
          promptAgain: true,
        },
      ]

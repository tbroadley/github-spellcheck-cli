...
function applySpellingCorrection({ index: { start, end }, suggestions: [suggestion] }, document) {
  return [
    ...
    _.size(suggestion) > 0 ? suggestion : '[no suggestion found]',
    document.slice(end),
  ].join('');
}
...
exports.addByUserSelection = async (misspellings, repo) => {
  ...
  for (let index = 0; index < misspellings.length; index += 1) {
    ...
    if (_.includes(wordsToSkip, misspelling.misspelling) || _.includes(filesToSkip, misspelling.path)) {
      continue;
    }
    ...
    await respondToUserInput(
      'Apply this correction? y(es), n(o), i(gnore word), e(dit), s(kip file): ',
      [
        {
          regex: /^y(es)?$/,

...
exports.addByUserSelection = async (misspellings, repo) => {
  ...
  for (let index = 0; index < misspellings.length; index += 1) {
    ...
    if (!skipWord(misspelling, filePath, whitelistedWords, filesToSkip)) {
      ...
      const remainingMisspellings = misspellings.slice(index + 1);
      const remainingThisWord = _.filter(remainingMisspellings, { misspelling }).length;
      const sOrNot = remainingThisWord === 1 ? '' : 's';

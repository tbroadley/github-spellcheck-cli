...
async function readFileToCorrect(relativePath, repo) {
  const contents = await h(fs.readFile(getAbsolutePath(relativePath, repo), { encoding: 'utf8' }));
...
exports.addByUserSelection = async (misspellings, repo) => {
  ...
  for (let index = 0; index < misspellings.length; index += 1) {
    ...
    const currentFileContents = await h(readFileToCorrect(misspelling.path, repo));
    ...
    const updateFile = async fileContents => {
      console.log('writing...');
      await h(fs.writeFile(getAbsolutePath(misspelling.path, repo), fileContents));
    ...
    await respondToUserInput(
      ...
      [
        ...
        },
        {
          regex: /^e(dit)?$/,
          responseFunction: async () => {
            const response = await h(prompt(`Replace "${misspelling.misspelling}" with: `));

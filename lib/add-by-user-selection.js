...
function remainingMisspellings(misspellings, index, whitelistedWords, filesToSkip) {
...
exports.addByUserSelection = async (misspellings, repo) => {
  ...
  for (let index = 0; index < misspellings.length; index += 1) {
    ...
    if (!skipWord(misspelling, filePath, whitelistedWords, filesToSkip)) {
      ...
      const remaining = remainingMisspellings(
        misspellings,
        index,
        whitelistedWords,
        filesToSkip
      ...
      const remainingThisWord = _.filter(remaining, { misspelling }).length;
      let sOrNot = remainingThisWord === 1 ? '' : 's';
      const output = `${remainingThisWord} instance${sOrNot} of "${misspelling}" after this.`;
      console.log(chalk.yellow(output));

      const remainingThisFile = _(remaining)
        .filter({ path: filePath })
        .uniq()
        .size();
      sOrNot = remainingThisFile === 1 ? '' : 's';
      console.log(chalk.yellow(`${remainingThisFile} mistake${sOrNot} remaining in this file.`));

      sOrNot = remaining.length === 1 ? '' : 's';
      console.log(chalk.yellow(`${remaining.length} mistake${sOrNot} remaining in total.`));

      const remainingFiles = _(remaining)
        .reject({ path: filePath })
        .map('path')
        .uniq()
        .size();
      console.log(chalk.yellow(`${remainingFiles} file${remainingFiles === 1 ? '' : 's'} remaining.`));

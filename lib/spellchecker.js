...
function buildSpellchecker({
  ...
  const frontmatterOptions = plugins.filter(({ frontmatter }) => frontmatter);
  if (frontmatterOptions.length > 0) {
    spellchecker.use(frontmatter);
    spellchecker.use(frontmatterFilter, frontmatterOptions[0].frontmatter);
  }
